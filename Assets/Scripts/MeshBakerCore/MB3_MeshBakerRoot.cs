...
public abstract class MB3_MeshBakerRoot : MonoBehaviour
{
	...
	public static bool DoCombinedValidate(MB3_MeshBakerRoot mom, MB_ObjsToCombineTypes objToCombineType, MB2_EditorMethodsInterface editorMethods, MB2_ValidationLevel validationLevel)
	{
		...
		for (int i = 0; i < objectsToCombine.Count; i++)
		{
			...
			if (gameObject == null)
			{
				Debug.LogError($"The list of objects to combine contains a null at position {i}. Select and use [shift + delete] to remove the object, or purge all null objects from the context menu.", mom);
				return false;
			}
			for (int j = i + 1; j < objectsToCombine.Count; j++)
			{
				...
				{
					Debug.LogError("The list of objects to combine contains duplicates at " + i + " and " + j, mom.gameObject);
					return false;
				}
			}
